<x-app-layout>
    <x-slot name="pageTitle">Create Fee Record</x-slot>
    ...
    <div class="max-w-2xl mx-auto space-y-8">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="font-poppins text-2xl font-bold text-on-surface tracking-tight">Generate New Billing</h1>
                <p class="text-sm font-semibold text-on-surface-variant mt-0.5">Initialize a new financial record for a student.</p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-[40px] p-10 shadow-soft border border-outline-variant/10 overflow-hidden relative">
            ...
            <form action="{{ route('admin.fees.store') }}" method="POST" class="relative space-y-8">
                @csrf
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="title" required placeholder="e.g. Annual Tuition Fee 2024" class="w-full px-6 py-4 bg-surface-container-low border-none rounded-3xl text-sm font-bold focus:ring-2 focus:ring-primary/20">
                    </div>

                    <div class="grid grid-cols-2 gap-6">
                        <div>
                            <label class="block text-[11px] font-black text-on-surface-variant uppercase tracking-[0.2em] mb-3">Amount (₹)</label>
                            <input type="number" name="amount" required placeholder="0.00" class="w-full px-6 py-4 bg-surface-container-low border-none rounded-3xl text-sm font-black focus:ring-2 focus:ring-primary/20 text-primary">
                        </div>
                        <div>
                            <label class="block text-[11px] font-black text-on-surface-variant uppercase tracking-[0.2em] mb-3">Due Date</label>
                            <input type="date" name="due_date" required class="w-full px-6 py-4 bg-surface-container-low border-none rounded-3xl text-sm font-bold focus:ring-2 focus:ring-primary/20">
                        </div>
                    </div>

                    <div>
                        <label class="block text-[11px] font-black text-on-surface-variant uppercase tracking-[0.2em] mb-3">Initial Status</label>
